...
function getFilteredLocations() {
    ...
    // Proximity filter - only apply if permission granted or not yet asked
    if (geolocationPermissionGranted !== false) {
        filtered = filtered.filter(loc => {
            const distance = getDistance(currentMapCenter, { lat: loc.lat, lng: loc.lng });
...
function generateTestLocations(count, center) {
    /**
     * Test helper: Generate N random locations around a center point
     * Usage in browser console:
     *   generateTestLocations(100, {lat: 10.73, lng: 106.70})
     *   applyFilters()
     * This will add 100 test locations to allLocations and redraw the map
     */
    const types = ['Sports', 'Fitness'];
    const vibes = ['Cozy', 'Industrial', 'Loud/Party', 'Minimalist', 'Hidden Gem', 'Instagrammable'];
    const testLocations = [];
    
    for (let i = 0; i < count; i++) {
        const offsetLat = (Math.random() - 0.5) * 0.2;
        const offsetLng = (Math.random() - 0.2) * 0.3;
        ...
        testLocations.push({
            ...
            caption: 'Test location for clustering performance',
            seedVibes: [vibes[Math.floor(Math.random() * vibes.length)]],
            videoURL: 'https://example.com',
            address: `Test Address ${i + 1}`,
            vibes: [vibes[Math.floor(Math.random() * vibes.length)]],
            curatorChoice: Math.random() < 0.1 // 10% chance of curator choice
        });
    }
    
    allLocations.push(...testLocations);
    console.log(`Added ${count} test locations. Total: ${allLocations.length}. Run applyFilters() to redraw.`);
    return testLocations;
}

function getSocialData(center) {
    // Generate seeded fallback locations based on center coordinates
    const baseLocations = [
        {
            id: 'loc_001',
            ...
            caption: 'Neon lights, live DJ set, and a packed dance floor with industrial brick walls.',
            seedVibes: ['Loud/Party', 'Instagrammable', 'Industrial'],
            videoURL: 'https://www.tiktok.com/search?q=neon_lounge',
            address: '123 Main St'
        },
        {
            id: 'loc_002',
            ...
            caption: 'Warm latte art, soft lighting, and a quiet hidden gem for remote work.',
            seedVibes: ['Cozy', 'Minimalist', 'Hidden Gem'],
            videoURL: 'https://www.tiktok.com/search?q=cozy_cafe',
        ...
        },
        {
            ...
            caption: 'Concrete textures, plated tasting menu, and a sleek camera-ready interior.',
            seedVibes: ['Instagrammable', 'Industrial'],
            videoURL: 'https://www.facebook.com/search?q=urban_kitchen',
            address: '789 Broadway'
        },
        {
            id: 'loc_004',
            name: 'Sunset Valley View',
            type: 'Fitness',
            lat: center.lat - 0.012,
            lng: center.lng + 0.018,
            
            caption: 'An underrated overlook with calm trails, sunset color, and a hidden scenic angle.',
            seedVibes: ['Cozy', 'Hidden Gem'],
            videoURL: 'https://www.tiktok.com/search?q=sunset_valley',
            address: '321 Ridge Rd'
        },
        {
            id: 'loc_005',
            name: 'Electric Pulse Club',
            type: 'Sports',
            lat: center.lat + 0.018,
            lng: center.lng + 0.008,
        
            caption: 'Bass-heavy crowd, bright LED walls, and nonstop party clips on every story.',
            seedVibes: ['Loud/Party', 'Instagrammable'],
            videoURL: 'https://www.tiktok.com/search?q=electric_pulse',
            address: '555 Performance Ave'
        },
        {
            id: 'loc_006',
            ...
            caption: 'A minimalist food hall with exposed steel beams and clean design details.',
            seedVibes: ['Industrial', 'Minimalist', 'Instagrammable'],
            videoURL: 'https://www.facebook.com/search?q=artisan_market',
            address: '888 Market St'
        },
        {
            id: 'loc_007',
            name: 'Serenity Wellness Café',
            type: 'Sports',
            lat: center.lat + 0.014,
            lng: center.lng - 0.012,
        
            caption: 'Minimal white walls, cozy seating, and a calm space for journaling.',
            seedVibes: ['Cozy', 'Minimalist'],
            videoURL: 'https://www.instagram.com/explore/tags/serenity_wellness/',
            address: '222 Zen Lane'
        },
        {
            id: 'loc_008',
            name: 'Street Food Revolution',
            type: 'Fitness',
            lat: center.lat - 0.02,
            lng: center.lng + 0.015,
        
            caption: 'Underrated street bites, crowd energy, and a colorful night-market atmosphere.',
            seedVibes: ['Loud/Party', 'Hidden Gem', 'Instagrammable'],
            videoURL: 'https://www.tiktok.com/search?q=street_food',
            address: '777 Alley Way'
        }
    ];

    return baseLocations.map(loc => ({
        ...loc,
        vibes: assignAIVibes(loc.caption, loc.seedVibes)
    }));
}
    ...
    return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
        <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
            ...
            <g filter="url(#shadow)">
                ...
                <text x="30" y="63" font-family="Segoe UI, Arial, sans-serif" font-size="16" font-weight="500" fill="#6b7280">${escapeXml(distanceText)}</text>
            </g>
        </svg>
    `)}`;
}
...
async function loadMapMarkers() {
    ...
    // Base mock/social-generated locations
    const generatedLocations = getSocialData(currentMapCenter);

    const remoteLocations = await loadServerLocations();
    if (remoteLocations.length > 0) {
        const remoteMap = new Map(remoteLocations.map(location => [location.id, location]));
        const mergedBase = generatedLocations.map(location => remoteMap.get(location.id) || location);
        const additions = remoteLocations.filter(location => !generatedLocations.some(base => base.id === location.id));
        allLocations = [...mergedBase, ...additions];
    } else {
        allLocations = generatedLocations;
    }
...
function setMobileOverlayState({ searchOpen = false, filtersOpen = false } = {}) {
    ...
    document.body.classList.toggle('mobile-filters-open', filtersOpen);

    if (sidebarBackdrop) {
        sidebarBackdrop.classList.toggle('visible', filtersOpen);
        sidebarBackdrop.setAttribute('aria-hidden', filtersOpen ? 'false' : 'true');
    }

    if (mobileSearchBtn) {
        mobileSearchBtn.classList.toggle('active', searchOpen);
    }

    if (mobileFiltersBtn) {
        mobileFiltersBtn.classList.toggle('active', filtersOpen);
    }
}
...
function syncSearchInputs(value) {
    ...
    if (mobileSearchInputElement && mobileSearchInputElement.value !== value) {
        mobileSearchInputElement.value = value;
    }
}
...
function setupMobileSidebarSwipeClose() {
    ...
    mobileSidebar.addEventListener('touchstart', (event) => {
        if (!isMobileViewport() || !document.body.classList.contains('mobile-filters-open')) {
            mobileSidebarTouchStart = null;
            return;
        }

        const touch = event.touches[0];
        mobileSidebarTouchStart = {
            x: touch.clientX,
            y: touch.clientY,
            time: Date.now()
        };
    ...
    mobileSidebar.addEventListener('touchend', (event) => {
        if (!mobileSidebarTouchStart || !isMobileViewport() || !document.body.classList.contains('mobile-filters-open')) {
            mobileSidebarTouchStart = null;
            return;
        }
        ...
        const elapsed = Date.now() - mobileSidebarTouchStart.time;

        mobileSidebarTouchStart = null;

        const isHorizontalSwipe = Math.abs(deltaX) > Math.abs(deltaY);
        const isFastEnough = elapsed < 800;
        const isLeftSwipe = deltaX < -60;

        if (isHorizontalSwipe && isFastEnough && isLeftSwipe) {
            closeMobileOverlays();
        }
    }, { passive: true });
}
...
function applyFilters() {
    ...
    updateVectorFeatures(filtered);
}

function getDistance(coord1, coord2) {
    // Simple distance calculation (in km, using approximate formula)
